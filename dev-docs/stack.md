...
- 코틀린 자체는 타입스크립트와 똑같기 때문에 같이 공부하면 오히려 도움이 될듯. 하지만 안드로이드스튜디오에 대한 학습비용이 너무 크다.
- 직업으로써의 주력 분야는 네트워크임. 모바일은 하드웨어도 좋아하기 때문에 그와 관련해서 흥미 위주로 관심이 있었음. 그런데 겪어보니까 흥미만으로 하기엔 너무 비용이 큰 것 같다.
- 때마침 PWA라는 개념을 알게 되어 웹 앱으로 하드웨어를 최대한 제어하는 것에 도전해보기로 함.
